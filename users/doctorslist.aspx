<%@ Page Title="" Language="C#" MasterPageFile="~/users/MasterPage.master" AutoEventWireup="true" CodeFile="doctorslist.aspx.cs" Inherits="users_Default" %>
...
<br />
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:vetliveConnectionString %>" SelectCommand="SELECT registration.name AS a, registration.email, hospitals.hospital_name FROM registration INNER JOIN login ON registration.email = login.email AND login.usertype = 'doctor' INNER JOIN staff ON registration.email = staff.email INNER JOIN hospitals ON staff.hospital_id = hospitals.hospital_id"></asp:SqlDataSource>
